#' SLIP with thresholding (c version)
#'
...
#' @param C tuning parameter for mean screening; [1.4, 1.6] is suitable;
#'     larger \code{p}, bigger \code{C}
#' @param outputW logical parameter \code{FALSE}(default); if \code{TRUE}, the
...
#'     \item{W}{W-statistic, optional only when \code{outputW = TRUE}}
#'     \item{L}{threshold, optional only when \code{outputW = TRUE}}
...
#'   SLIP.thresh.c(data$dat, 0.1)
#'
SLIP.thresh.c <- function(dat, alpha, C = 1.5, r=3, covEst = T,
  ...
  if (covEst){
    ...
    if (estMthd == "Cholesky"){
      Sig = chol2inv(chol(PreEst.2017Lee(dat_c, upperK = 10L)$C))
